...
{
    // 여기를 완성하세요
    int i,j; // index 나타내기 위한 변수
    int cnt[9]; // 스토쿠 각 행에 있는 숫자 개수를 check 하는 배열
    for(i=0; i<9; i++){
      for(j=0; j<9; j++){
        cnt[j]=0;
      } // 스도쿠에 들어있는 숫자 개수 초기화
      ...
      for(j=0; j<9; j++){
        if(cnt[j]!=1){
          valid[0][i]=0; // 각 행의 각 숫자가 한 개가 아니면 0 표시 후 break
          break;
        }
      }
    }
    pthread_exit(NULL); // 스레드를 종료해준다.
}
...
void *check_columns(void *arg)
{
    // 여기를 완성하세요
    int i,j; // index 나타내기 위한 변수
    int cnt[9]; // 스토쿠 각 열에 있는 숫자 개수를 check 하는 배열
    for(i=0; i<9; i++){
      for(j=0; j<9; j++){
        cnt[j]=0;
      } // 스도쿠에 들어있는 숫자 개수 초기화
      ...
      for(j=0; j<9; j++){
        if(cnt[j]!=1){
          valid[1][i]=0; // 각 열의 각 숫자가 한 개가 아니면 0 표시 후 break
          break;
        }
      }
    }
...
{
    // 여기를 완성하세요
    int i,j,k; // index를 위한 변수 i,j와 서브그리드 번호인 k를 선언
    int row,col; // 각 서브그리드 index 시작점 표시 위한 변수
    int cnt[9]; // 스토쿠 각 서브그리드에 있는 숫자 개수를 check 하는 배열

    k = *(int*)arg; // 서브그리드 번호를 나타내는 void *arg를 int로 변환해 k에 넣어준다.
    ...
    for(int i=0; i<9; i++){
      if(cnt[i]!=1){
        valid[2][k]=0; // 서브그리드의 각 숫자가 한 개가 아니면 0 표시 후 break
        break;
      }
    }
    pthread_exit(NULL); // 스레드를 종료해준다.
...
void check_sudoku(void)
{
     ...
     */
    // 여기를 완성하세요
    if(pthread_create(&rowtid, NULL, check_rows, NULL) != 0){ // 각 행을 검사하는 스레드를 생성합니다.
     ...
     */
    // 여기를 완성하세요
    if(pthread_create(&coltid, NULL, check_columns, NULL) != 0){ // 각 열을 검사하는 스레드를 생성합니다.
     ...
     */
    // 여기를 완성하세요
    pthread_join(rowtid, NULL); // 각 열을 검사하는 스레드를 종료할 때까지 기다린다.
    pthread_join(coltid, NULL); // 각 열을 검사하는 스레드를 종료할 때까지 기다린다.
    for(i=0; i<9; i++){ // 각 서브그리드를 검사하는 스레드를 종료할 때까지 기다린다.
